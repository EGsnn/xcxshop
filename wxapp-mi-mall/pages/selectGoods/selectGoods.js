...
Page({
  ...
  submit(e) {
    ...
    if (pre_item.length>0){
      var goods_storage = pre_item.map(obj => {
        if (obj.goods_id == this.data.goods_attrSelect.goods_id) {
          obj.select_num = obj.select_num + this.data.select_num;
          flag = 0;
        }
        return obj;
      });
    ...
    if (flag){
      const temp = {
        'goods_name': this.data.goods_attrSelect.goods_name,
        'goods_price': this.data.goods_attrSelect.goods_price,
        'imgs': this.data.goods_attrSelect.imgs,
        'goods_id': this.data.goods_attrSelect.goods_id,
        'select_num': this.data.select_num,
        'selected': true,
        'isTouchMove': false
      }
      ...
      wx.setStorageSync('attr_item', [...goods_storage]);
  ...
  onLoad: function (options) {
    ...
    util.httpPost('goods/detail', { goods_id: id }).then((res) => {
      console.log(res.resultList)
      this.setData({
        goods_attrSelect: res.resultList
      });
